...
def isTerminal(board):  # Checks if one Player won or no Moves are left
    return winningMove(board, AI1_PIECE) or winningMove(board, AI2_PIECE) or len(getAllMoves(board)) == 0
...
def minimax(board, depth, alpha, beta, maximizingPlayer):
    ...
    if depth == 0 or is_terminal:  # Break condition
        if is_terminal:
            if winningMove(board, AI2_PIECE):
            ...
            return (None, evaluate(board, AI2_PIECE))  # Return the Score
    ...
    if maximizingPlayer:
        ...
        for col in validLocations:  # Go through all the possible Colums
            ...
            b_copy = board.copy()
            doMove(b_copy, row, col, AI2_PIECE)  # Drop the AI piece on the temp Board
    ...
    while not gameOver:  # Game Loop
        ...
        if turn == PLAYER and not gameOver:
            ...
            if possibleMove(board, col):
                ...
                turn += 1
                turn = turn % 2  # Turn even => AI1, Turn odd => AI2
        ...
        if turn == AI and not gameOver:
            ...
            if possibleMove(board, col):
                row = getNextOpenRaw(board, col)
                doMove(board, row, col, AI2_PIECE)

                if winningMove(board, AI2_PIECE):
                    wincounterAI2 = wincounterAI2 + 1
                    print("AI 2 Sieg mit folgender Stellung:")
                    printTerminal(board)
                    gameOver = True
